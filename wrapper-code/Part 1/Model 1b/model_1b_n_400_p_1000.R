...
generate_model_1b <- function(n = 400, p = 1000, seed = NULL) {
  ...
  # Extract the first 7 columns to construct Y
  ...
  # Generate Y (Model 1.b)
  # For numerical stability, add a very small lower bound to (1 - X5) inside the log; remove if not needed
...
alpha = 0.5
if(alpha==0.5){
  active.set=c(1, 2, 3, 4, 5)
}else{
  active.set=c(1, 2, 3, 4, 5, 6, 7)
}
...
save(vars, times, file = paste0("model2_results_and_times_n", n, "_p", p, "_alpha", alpha_str, ".RData"))

# Convert to data frames and save
result_df <- data.frame(Metric = names(result_table), Value = result_table)
time_df <- data.frame(Metric = names(time_table), Value = time_table)

write.csv(result_df, paste0("model2_results_n", n, "_p", p, "_alpha", alpha_str, ".csv"), row.names = FALSE)
write.csv(time_df, paste0("model2_time_n", n, "_p", p, "_alpha", alpha_str, ".csv"), row.names = FALSE)
