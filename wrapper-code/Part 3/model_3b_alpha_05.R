...
PCSIS <- function(Y, X, gammas, active_set,
  ...
  # 也给个简表方便查看
  summary_df <- data.frame(
    gamma = vapply(results_list, function(z) z$gamma, numeric(1)),
    tau   = vapply(results_list, function(z) z$tau,   numeric(1)),
    m     = vapply(results_list, function(z) z$m,     integer(1)),
    selected_size = vapply(results_list, function(z) length(z$selected_idx), integer(1))
  )
  
  list(
    summary = summary_df,         # (tau, m, number of selected features) for each gamma
...
KCQD <- function(Y, X, alpha, active_index, gammas) {
  ...
  summary_df <- data.frame(
    ...
    tau   = vapply(details_list, function(z) z$tau,   numeric(1)),
    m     = vapply(details_list, function(z) z$m,     integer(1)),
    selected_size_features = vapply(details_list, function(z) length(z$selected_features), integer(1))
  )
...
run_one_block <- function(block_id, seed) {
  ...
  for (r in 1:nrep_block) {
    ...
    data <- generate_model_2b(n = n, p = p, n_active)
    X <- data$X
    Y <- data$Y
...
saveRDS(summary_results, file = "model_3b_alpha_05.rds")
