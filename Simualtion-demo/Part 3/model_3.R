...
PCSIS <- function(Y, X, gammas, active_set,
  ...
  list(
    ...
    res = res,                    # difference scores, useful for diagnostics/plots
    top_idx = top_idx             # original indices of the top 50 variables
  )
}
...
u4_stat_rowgroups_mean <- function(X, Y, tau,
  ...
  # For each row group, run once and get the 50 column-group u4 statistics
  ...
  for (g in seq_along(row_groups)) {
    ...
    # Inner parallelization over column groups (avoid nested over-subscription)
    out <- u4_stat_groups_parallel(
      Xg, Yg, tau, beta,
      groups = col_groups,          # explicitly pass the 50 column groups
      n_workers = n_workers_col     # number of workers for column-group parallelization
    )
    # out is a numeric vector: u4 statistics for each group
    res_mat[g, ] <- out
  }
  
  # Average across the 20 row groups in the column direction to get a 50×1 vector
  colMeans(res_mat)
}
...
KCQD <- function(Y, X, alpha, active_index, gammas) {
  ...
  # 2) First stage: compute group statistics and retain them (length of T_vals = number of groups)
  ...
  solve_for_gamma <- function(g) {
    ...
    # 3.2 Expand selected groups into corresponding original column indices
    ...
    # 3.3 Second stage: compute statistics for selected groups (note n_col_groups is number of groups, not number of columns)
    ...
    # The length of s_vec should equal the number of selected groups
    if (length(s_vec) != n_groups2) {
      stop("Length mismatch: s_vec should equal the number of selected groups; please check return of u4_stat_rowgroups_mean.")
    }
...
############## simulation 2a alpha=0.5 ##############
generate_model_2a <- function(n = 400, p = 2000, n_active) {
  ...
  # Generate response Y
  Y <- 5 * rowSums(X[,1:n_active]) + epsilon
...
## -------- Initialize containers for all repetitions --------
sel_sizes_pc    <- matrix(0L,    nrow = nrep_total, ncol = G,
                          dimnames = list(NULL, paste0("g=", gammas_vec)))
fdr_vals_pc     <- matrix(0.0,   nrow = nrep_total, ncol = G,
                          dimnames = list(NULL, paste0("g=", gammas_vec)))
all_selected_pc <- matrix(FALSE, nrow = nrep_total, ncol = G,
                          dimnames = list(NULL, paste0("g=", gammas_vec)))
sel_matrix_pc   <- array(FALSE, dim = c(nrep_total, p, G),
                         dimnames = list(NULL, NULL, paste0("g=", gammas_vec)))

sel_sizes_fs    <- matrix(0L,    nrow = nrep_total, ncol = G,
                          dimnames = list(NULL, paste0("g=", gammas_vec)))
fdr_vals_fs     <- matrix(0.0,   nrow = nrep_total, ncol = G,
                          dimnames = list(NULL, paste0("g=", gammas_vec)))
all_selected_fs <- matrix(FALSE, nrow = nrep_total, ncol = G,
                          dimnames = list(NULL, paste0("g=", gammas_vec)))
sel_matrix_fs   <- array(FALSE, dim = c(nrep_total, p, G),
                         dimnames = list(NULL, NULL, paste0("g=", gammas_vec)))

sel_sizes_KCQD    <- matrix(0L,    nrow = nrep_total, ncol = G,
                            dimnames = list(NULL, paste0("g=", gammas_vec)))
fdr_vals_KCQD     <- matrix(0.0,   nrow = nrep_total, ncol = G,
                            dimnames = list(NULL, paste0("g=", gammas_vec)))
all_selected_KCQD <- matrix(FALSE, nrow = nrep_total, ncol = G,
                            dimnames = list(NULL, paste0("g=", gammas_vec)))
sel_matrix_KCQD   <- array(FALSE, dim = c(nrep_total, p, G),
                           dimnames = list(NULL, NULL, paste0("g=", gammas_vec)))

time_fs   <- numeric(nrep_total)
time_pc   <- numeric(nrep_total)
time_kcqd <- numeric(nrep_total)

## -------- Main loop over all repetitions --------
for (r in 1:nrep_total) {
  cat(sprintf("Rep %d / %d\n", r, nrep_total))
  
  ## Step 1: generate data
  data <- generate_model_2a(n = n, p = p, n_active = n_active)
  X <- data$X
  Y <- data$Y
  
  ## Step 2: FS method
  t0 <- proc.time()
  comp <- compute_D_dagger_from_alpha(
    X, Y,
    h = h,
    alpha = alpha,
    delta_n_alpha = delta_n_alpha
  )
  res_fs <- adaptive_threshold_select(comp$D_dagger, gammas = gammas_vec)
  time_fs[r] <- (proc.time() - t0)[["elapsed"]]
  
  for (g in seq_len(G)) {
    S_hat <- res_fs$details[[g]]$selected_idx
    ...
    if (length(S_hat) > 0) {
      sel_matrix_fs[r, S_hat, g] <- TRUE
    }
  }
  
  ## Step 3: PC method
  t0 <- proc.time()
  res_pc <- PCSIS(Y, X, gammas = gammas_vec, active_set = 1:n_active)
  time_pc[r] <- (proc.time() - t0)[["elapsed"]]
  
  for (g in seq_len(G)) {
    S_hat <- res_pc$details[[g]]$selected_idx
    ...
    if (length(S_hat) > 0) {
      sel_matrix_pc[r, S_hat, g] <- TRUE
    }
  }
  
  ## Step 4: KCQD method
  t0 <- proc.time()
  res_kcqd <- KCQD(Y, X, alpha, 1:10, gammas_vec)
  time_kcqd[r] <- (proc.time() - t0)[["elapsed"]]
  
  for (g in seq_len(G)) {
    S_hat <- res_kcqd$details[[g]]$selected_idx
    ...
    if (length(S_hat) > 0) {
      sel_matrix_KCQD[r, S_hat, g] <- TRUE
    }
  }
}
...
# FS
median_size_fs <- apply(sel_sizes_fs, 2, median)
Pj_fs <- sapply(1:G, function(g) {
  colMeans(sel_matrix_fs[, 1:n_active, g, drop = FALSE])
})
prob_all_fs <- colMeans(all_selected_fs)
emp_fdr_fs  <- colMeans(fdr_vals_fs)
...
# PC
median_size_pc <- apply(sel_sizes_pc, 2, median)
Pj_pc <- sapply(1:G, function(g) {
  colMeans(sel_matrix_pc[, 1:n_active, g, drop = FALSE])
})
prob_all_pc <- colMeans(all_selected_pc)
emp_fdr_pc  <- colMeans(fdr_vals_pc)
...
# KCQD
median_size_kcqd <- apply(sel_sizes_KCQD, 2, median)
Pj_kcqd <- sapply(1:G, function(g) {
  colMeans(sel_matrix_KCQD[, 1:n_active, g, drop = FALSE])
})
prob_all_kcqd <- colMeans(all_selected_KCQD)
emp_fdr_kcqd  <- colMeans(fdr_vals_KCQD)
...
summary_results <- list(
  FS   = list(
    median_size = median_size_fs,
    Pj = Pj_fs,
    prob_all = prob_all_fs,
    emp_fdr = emp_fdr_fs,
    time = time_fs
  ),
  PC   = list(
    median_size = median_size_pc,
    Pj = Pj_pc,
    prob_all = prob_all_pc,
    emp_fdr = emp_fdr_pc,
    time = time_pc
  ),
  KCQD = list(
    median_size = median_size_kcqd,
    Pj = Pj_kcqd,
    prob_all = prob_all_kcqd,
    emp_fdr = emp_fdr_kcqd,
    time = time_kcqd
  )
)
...
############## simulation 2b alpha=0.5 ##############
generate_model_2b <- function(n = 400, p = 2000, n_active) {
  # Set covariance matrix Σ_{ij} = 0.8^{|i-j|}
  Sigma <- outer(1:p, 1:p, function(i, j) 0.8^abs(i - j))
  ...
  # Generate Y = 5 * sum_{j=1}^{10} exp(0.5 * X_j) + ε
  Y <- 5 * rowSums(exp(0.5 * X[, 1:n_active])) + epsilon
...
for (r in 1:nrep_total) {
  ...
  ## Step 1: generate data
  data <- generate_model_2b(n = n, p = p, n_active = n_active)
  X <- data$X
  Y <- data$Y
...
for (r in 1:nrep_total) {
  ...
  ## Step 1: generate data
  data <- generate_model_2c(n = n, p = p, n_active = n_active)
  X <- data$X
  Y <- data$Y
