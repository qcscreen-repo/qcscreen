...
KCQD <- function(Y, X, alpha, active_index, gammas) {
  ...
  solve_for_gamma <- function(g) {
    ...
    list(
      gamma = g, tau = tau, m = m,
      selected_idx = selected_in_svec,     # indices relative to s_vec
      selected_features = selected_features # original column indices
    )
  }
...
## -------- Split 100 repetitions into 5 blocks, each with 20 reps --------
n_blocks <- 5
nrep_block <- nrep_total / n_blocks   # 20
stopifnot(nrep_block == floor(nrep_block))


## -------- Function to run one block: return all results for this block --------
run_one_block <- function(block_id, seed) {
  ...
  for (r in 1:nrep_block) {
    ...
    ## Step 4: KCQD method
    t0 <- proc.time()
    res <- KCQD(Y, X, alpha, 1:10, gammas_vec)
...
## Helper: row-bind matrix fields
bind_mtx <- function(field) do.call(rbind, lapply(blocks, `[[`, field))

## Helper: concatenate 3D arrays along the first (rep) dimension
bind_arr <- function(field) {
  arrs <- lapply(blocks, `[[`, field)
  stopifnot(length(unique(lapply(arrs, function(a) dim(a)[2]))) == 1)  # consistent p across arrays
  stopifnot(length(unique(lapply(arrs, function(a) dim(a)[3]))) == 1)  # consistent G across arrays
...
## Combine to obtain final results over 100 repetitions
...
saveRDS(summary_results, file = "model_3a_alpha_075.rds")

# Extract summary_results and organize into tables
library(dplyr)
library(tidyr)
library(purrr)
