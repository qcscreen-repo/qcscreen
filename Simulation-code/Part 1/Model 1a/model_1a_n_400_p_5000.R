...
## Data-generating function (model 1a)
...
## Formatting helper (two decimal places)
fmt_median_iqr <- function(x) sprintf("%.2f (%.2f)", stats::median(x, na.rm = TRUE), IQR(x, na.rm = TRUE))

## Recursively flatten nested lists and summarize
...
## Initialize result/time containers
init_containers <- function() {
  ...
  list(results = results, times = times)
}

## Run a single chunk: rep_per_chunk iterations
run_chunk <- function(chunk_id, seed, n, p, rep_per_chunk, active.set) {
  set.seed(seed)
  containers <- init_containers()
  results <- containers$results; times <- containers$times
  ...
  for (i in seq_len(rep_per_chunk)) {
    ...
    cat(sprintf("chunk %d | iter %d/%d done\n", chunk_id, i, rep_per_chunk))
  }
  
  # Each chunk saves its own RData (for easy checkpointing)
  save_path <- sprintf("model1_chunk_%d_seed_%s.RData", chunk_id, seed)
  save(results, times, n, p, rep_per_chunk, seed, file = save_path)
  invisible(save_path)
}

run_chunk(chunk_id = 1, seed = 1234, n = n, p = p, rep_per_chunk = rep_per_chunk, active.set)
run_chunk(chunk_id = 2, seed = 1235, n = n, p = p, rep_per_chunk = rep_per_chunk, active.set)
run_chunk(chunk_id = 3, seed = 1236, n = n, p = p, rep_per_chunk = rep_per_chunk, active.set)
run_chunk(chunk_id = 4, seed = 1237, n = n, p = p, rep_per_chunk = rep_per_chunk, active.set)
run_chunk(chunk_id = 5, seed = 1238, n = n, p = p, rep_per_chunk = rep_per_chunk, active.set)

## Paths and helper tools
fmt_median_iqr <- function(x) sprintf("%.2f (%.2f)", stats::median(x, na.rm = TRUE), IQR(x, na.rm = TRUE))
...
## Recursively merge nested lists (concatenate leaf vectors with the same name)
merge_nested <- function(a, b) {
  if (is.null(a)) return(b)
  if (is.null(b)) return(a)
  if (is.list(a) && is.list(b)) {
    keys <- union(names(a), names(b))
    out <- vector("list", length(keys)); names(out) <- keys
    for (k in keys) out[[k]] <- merge_nested(a[[k]], b[[k]])
    return(out)
  } else {
    return(c(a, b))  # leaf: concatenate vectors
  }
}

## Read all chunk RData files and merge
files <- list.files(pattern = "^model1_chunk_\\d+_seed_.*\\.RData$", full.names = TRUE)
...
for (f in files) {
  env <- new.env()
  load(f, envir = env)
  results_all <- merge_nested(results_all, env$results)
  times_all   <- merge_nested(times_all,   env$times)
}

## Generate summary tables
result_table <- summarize_nested(results_all)
time_table   <- summarize_nested(times_all)

result_df <- data.frame(Metric = names(result_table), Value = unname(result_table))
time_df   <- data.frame(Metric = names(time_table),   Value = unname(time_table))

## Optional: sort metrics (those without alpha first, then a05/a075)
...
## Save final merged RData and CSV
save_path_rdata <- file.path(sprintf("model1_merged_n%d_p%d.RData", n, p))
save(results_all, times_all, result_table, time_table, file = save_path_rdata)

write.csv(result_df, file.path(sprintf("model1_results_n%d_p%d.csv", n, p)), row.names = FALSE)
write.csv(time_df,   file.path(sprintf("model1_time_n%d_p%d.csv",    n, p)), row.names = FALSE)

## Print summary and path of merged RData
print(result_table)
print(time_table)
cat("Merged RData:", save_path_rdata, "\n")
